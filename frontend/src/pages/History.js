            ...
            {transactions.length === 0 ? (
                ...
                <table>
                    ...
                    {transactions.map((tx, idx) => {
                        ...
                        const amount = parseFloat(tx.units ?? tx.amount ?? 'NaN');
                        const price = parseFloat(tx.pricePerUnit ?? tx.price_per_unit ?? 'NaN');
                        const total = parseFloat(tx.total ?? amount * price ?? 'NaN');
                        ...
                        return (
                            <tr key={idx}>
                                ...
                                <td>{!isNaN(total) ? `${isBuy ? '- ' : '+ '}$${total.toFixed(2)}` : 'N/A'}</td>
                            </tr>
                        );
                    })}
                    </tbody>
